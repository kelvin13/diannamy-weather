...
class CairoSurface
{
    ...
    public convenience
    init?(format:CairoFormat, width:CInt, height:CInt)
    {
        self.init(c_surface: cairo_image_surface_create(format.c_format, width, height))
    }
    ...
    public
    func with_data<Result>(_ f:(UnsafeMutableBufferPointer<UInt8>) -> Result) -> Result
    {
