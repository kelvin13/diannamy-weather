...
enum Shaders
{
    ...
    let vertcolor = Shader(vertex_file  : "shaders/vertcolor.vert",
                           ...
                           uniforms     : ["model"])!
    static
    let cloudgen_shield  = Shader(vertex_file: "shaders/cloudgen.vert",
                           geometry_file: "shaders/cloudgen-shield.geom",
                           fragment_file: "shaders/cloudgen-shield.frag",
                           tex_uniforms : ["tex_cloud"],
                           uniforms     : ["model"])!
    static
    let cloudgen_kernel  = Shader(vertex_file  : "shaders/cloudgen.vert",
                           geometry_file: "shaders/cloudgen-kernel.geom",
                           fragment_file: "shaders/cloudgen-kernel.frag",
                           tex_uniforms : ["tex_cloud"],
                           uniforms     : ["model"])!
    static
    let globe     = Shader(vertex_file  : "shaders/planet.vert",
                           fragment_file: "shaders/planet.frag",
...
class FlowSphere
{
    ...
    {
        // globe
        glUseProgram(Shaders.vertcolor.program)
        glUniformMatrix4fv(Shaders.vertcolor.u_ids[0], 1, false, self.transform.model_matrix)
        //glBindVertexArray(self.field_mesh.VAO)
        //glDrawElements(GL.LINES, self.field_mesh.n, GL.UNSIGNED_INT, nil)

        glUseProgram(Shaders.globe.program)
        ...
        glBindTexture(GL.TEXTURE_CUBE_MAP, self.globe_tex.tex_id)

        glBindVertexArray(self.ball_mesh.VAO)
        glDrawElements(GL.TRIANGLES, self.ball_mesh.n, GL.UNSIGNED_INT, nil)

        // clouds
        glDisable(GL.DEPTH_TEST)
        glBindVertexArray(self.point_mesh.VAO)

        glUseProgram(Shaders.cloudgen_shield.program)
        ...
        glActiveTexture(GL.TEXTURE0)
        glBindTexture(GL.TEXTURE_2D, self.cloud_tex_shield.tex_id)
        glDrawElements(GL.POINTS, self.point_mesh.n, GL.UNSIGNED_INT, nil)
...
struct View3D:GameScene
{
    ...
    private
    var render_mode:GL.Enum = GL.FILL,
        zoom_level:Int = 8,
    ...
    func show3D(_ dt:Double)
    {
        ...
        glEnable(cap: GL.CULL_FACE)
        glPolygonMode(GL.FRONT_AND_BACK, self.render_mode)
        ...
        cr.set_font_size(13)
        cr.show_text("\(Int((1/dt).rounded())) FPS (render mode: \(self.render_mode == GL.FILL ? "faces" : "wireframe"))")
    ...
    func key(_ key:PhysicalKey)
    {
        ...
        {
        case .tab:
            self.render_mode = self.render_mode == GL.FILL ? GL.LINE : GL.FILL
        case .space:
