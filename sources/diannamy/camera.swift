...
struct UniformCameraResource
{
    ...
    {
        var matrix_buffer:GL.UInt = 0
        glGenBuffers(1, &matrix_buffer)
        glBindBuffer(GL.UNIFORM_BUFFER, matrix_buffer)
        // allocate space for the buffer
        glBufferData(GL.UNIFORM_BUFFER, 168, nil, GL.DYNAMIC_DRAW)
        // bind camera buffer to position 0
        glBindBufferBase(GL.UNIFORM_BUFFER, 0, matrix_buffer)
        glBindBuffer(GL.UNIFORM_BUFFER, 0)
    ...
    func apply_camera()
    {
        glBindBuffer(GL.UNIFORM_BUFFER, self.matrix_buffer)
        glBufferSubData(GL.UNIFORM_BUFFER, 0, 64, self.projection_matrix)
        glBufferSubData(GL.UNIFORM_BUFFER, 64, 64, self.view_matrix)
        glBufferSubData(GL.UNIFORM_BUFFER, 128, 12, [self.position.x, self.position.y, self.position.z])
        glBufferSubData(GL.UNIFORM_BUFFER, 144, 24, [0.5*self.h, 0.5*self.k, 2*self.size, self.z, self.shift_x, self.shift_y])
        glBindBuffer(GL.UNIFORM_BUFFER, 0)
    }
}
